...
- First server sync scaffold: the server implements authenticated `server.nodes.mkdir`, and the daemon exposes `sync.status`, `sync.push`, and `sync.pull` for HTTP v1 server namespace/content round trips with conservative base-version checks and local-divergence skips.
