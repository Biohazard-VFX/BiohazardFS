...
- First server sync scaffold: the server implements authenticated `server.nodes.mkdir`, and the daemon exposes `sync.status`, `sync.push`, and `sync.pull` for HTTP v1 server namespace/content round trips with conservative base-version checks and local-divergence skips. `sync.push` and `sync.pull` are classified `LowRisk` so the desktop can drive them directly without an operation token.
- FUSE workspace mount now re-fetches directory children after a short TTL (`CHILDREN_CACHE_TTL`) so daemon-side mutations (sync pull/push, collaborator changes) appear in the mount without a remount.
- `ci: add server-image workflow` publishes `ghcr.io/biohazard-vfx/biohazardfs-server` via `workflow_dispatch` (PAT `CR_PAT` secret, `GITHUB_TOKEN` fallback).
- Biohazard Workspace desktop now ships a **Server sync** Settings panel: the desktop stores an owner-only `server-sync.json` (URL + bearer token) under its userData directory, injects `BIOHAZARDFS_SERVER_URL` / `BIOHAZARDFS_SERVER_TOKEN` / `BIOHAZARDFS_ALLOW_PLAINTEXT_SERVER` into the managed daemon's environment on every startup, and exposes a Save-and-apply action that bounces the daemon + FUSE mount so the new config takes effect immediately. The panel also provides Push / Pull / Status buttons that drive daemon `sync.push` / `sync.pull` / `sync.status` against the configured control-plane server.
